...
internal static partial class Program
{
	...
	internal static void Update()
	{
		...
		foreach (IEnemy enemy in enemies)
		{
			var (sprite, offset) = player.GetSpriteAndOffset();
			for (int y = 0; y < sprite.Length; y++)
			{
				int yo = (int)player.Y + y + offset.Y;
				if (yo < 0 || yo >= Program.frameBuffer.GetLength(1)) continue;

				for (int x = 0; x < sprite[y].Length; x++)
				{
					int xo = (int)player.X + x + offset.X;
					if (xo < 0 || xo >= Program.frameBuffer.GetLength(0)) continue;

					if (sprite[y][x] != ' ' && enemies.Exists(e => e.CollidingWith(xo, yo)))
					{
						isDead = true;
						return;
					}
