...
void GetDirection()
{
	...
	if (newDirection.HasValue && !IsOpposite(newDirection.Value, previousDirection))
	{
		direction = newDirection;
	}
}

bool IsOpposite(Direction newDir, Direction? oldDir)
{
	if (!oldDir.HasValue) return false;

	return (newDir == Direction.Up && oldDir == Direction.Down) ||
		   (newDir == Direction.Down && oldDir == Direction.Up) ||
		   (newDir == Direction.Left && oldDir == Direction.Right) ||
		   (newDir == Direction.Right && oldDir == Direction.Left);
}
